...
* NEXT Week 8: Word clouds and visualization
#+attr_latex: :width 400px
[[../img/lotr.jpg]]
...
1) [ ] What is the process from CSV file to document corpus?
   #+begin_notes
...
2) [ ] How do you load the package ~tm~ and check if it contains a
...
3) [ ] What data structure is ~stopwords("en")~, and how many words does
   it contain?
   #+begin_src R
     str(stopwords("en"))  ## a character vector
   #+end_src

4) [ ] How can you find out if a particular word, say "cannot", is
   contained in the stopwords English dictionary?
   #+begin_src R
     any(stopwords("en")=="cannot")
   #+end_src

5) [ ] How would you add two new words like "coffee" and "bean" to the
   END of the stopwords English dictionary, and then check for them?
   #+begin_src R
     c(stopwords("en"),"coffee","bean") |> tail()
     tail(c(stopwords("en"),"coffee","bean"))
   #+end_src

6) [ ] Why does ~which(c(20,30,40)==30~ return the value ~2~ ?
   #+begin_src R
...
* TODO Week 9: Word clustering and dendograms
** Review: week 8
